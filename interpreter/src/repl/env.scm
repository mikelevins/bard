...
(%defglobal 'odd? odd?)
(%defglobal 'even? even?)
(%defglobal 'random (lambda (n)(random-integer n)))
...
(%defglobal '<input-stream> <input-stream>)
(%defglobal '<output-stream> <output-stream>)
...
(%defglobal 'print bard:print)
(%defglobal 'read bard:read)
