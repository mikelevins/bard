...
;; let's at least inline
(define (null-list? l)
  (cond ((instance? l <pair>) #f)
	((eq? l '()) #t)
	(else (error "null-list?: argument out of domain" l))))
...
(define (take-right lis k :: <integer>)
  (let lp ((lag lis)  (lead (drop lis k)))
    (if (pair? lead)
	(lp (cdr lag) (cdr lead))
	lag)))

(define (drop-right lis k :: <integer>)
...
(define-private (%cars+ lists last-elt)	; (append! (map car lists) (list last-elt))
  (let recur ((lists lists))
...
(define-private (%cars+cdrs lists)
  (call-with-current-continuation
    (lambda (abort)
      (let recur ((lists lists))
        (if (pair? lists)
...
(define-private (%cars+cdrs/pair lists)
...
(define-private (%cars+cdrs/no-test lists)
  (let recur ((lists lists))
    (if (pair? lists)
	(receive (list other-lists) (car+cdr lists)
	  (receive (a d) (car+cdr list)
...
;;; count
;;;;;;;;;
(define (count pred :: <procedure> list1 . lists)
  (if (pair? lists)
...
(define (unfold-right p :: <procedure> f :: <procedure> g :: <procedure> seed #!optional (maybe-tail '()))
...
(define (unfold p :: <procedure> f :: <procedure> g :: <procedure> seed . maybe-tail-gen)
...
(define (fold-right kons :: <procedure> knil lis1 . lists)
  (if (pair? lists)
...
(define (pair-fold-right f :: <procedure> zero lis1 . lists)
  (if (pair? lists)
      (let recur ((lists (cons lis1 lists)))		; N-ary case
	(let ((cdrs (%cdrs lists)))
	  (if (null? cdrs) zero
...
(define (reduce-right f :: <procedure> ridentity lis)
  (if (null-list? lis) ridentity
...
(define (map! f :: <procedure> lis1 . lists)
  (if (pair? lists)
...
; (define (map-in-order f :: <procedure> lis1 . lists)
;   (if (pair? lists)
