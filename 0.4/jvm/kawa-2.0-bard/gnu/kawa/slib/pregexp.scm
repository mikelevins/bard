...
#|
;; This aims to be compatible with the pregexp
;; Portable Regular Expression" library.
;; It's not quite right, because the regular
;; expression syntax aren't quite the same.
;; This should be fixed in the pregexp constructor.

(import (kawa regex))
(export pregexp pregexp-match pregexp-match-positions
	pregexp-replace pregexp-replace* pregexp-split)

(define (pregexp (str :: string)) :: regex
  ;; FIXME should actually convert syntax
  (regex str))

(define (pregexp-match? re (str :: string)
		      #!optional (start :: int 0) (end :: int (str:length)))
  (let* ((rex :: regex (if (instance? re regex) re (pregex (re:toString))))
...
(define (pregexp-match re (str :: string)
		      #!optional (start :: int 0) (end :: int (str:length)))
  (regex-match (if (instance? re regex) re (pregexp (re:toString)))
	       str start end))

(define (pregexp-split re (str :: string))
  (regex-split (if (instance? re regex) re (pregexp (re:toString))) str))

(define (pregexp-match-positions re (str :: string)
				 #!optional (start :: int 0) (end :: int (str:length)))
  (regex-match-positions (if (instance? re regex) re (pregexp (re:toString)))
	       str start end))

;; Convert replacement string from pregexp syntax.
(define (pregexp-replacementstring-to-regex (str :: string)) :: string
  (let* ((sbuf :: java.lang.StringBuffer (java.lang.StringBuffer))
    ...
    (let loop ((i :: int 0))
	  ...
	  (let ((c1 (str:charAt i)))
		...
		(begin
		  (set! i (+ i 1))
		  (let ((c2 :: char (str:charAt i)))
		    (cond ((java.lang.Character:isDigit c2)
			   (sbuf:append (as char #\$))
			   (sbuf:append c2))
			  ((char-eq? c2 #\&)
			   (sbuf:append "$0"))
			  ;; Empty string.  A not-quite-correct implementation:
			  ;; just escape the next character. FIXME
			  ((char-eq? c2 #\$)
			   (sbuf:append c1))
			  (else
			   (sbuf:append c1)
...
(define (pregexp-replace re (str :: string) repl) :: string
  (let ((rex :: regex (if (instance? re regex) re (pregexp (re:toString)))))
    (regex-replace re str
		   (if (procedure? repl) repl
		       (pregexp-replacementstring-to-regex repl)))))
