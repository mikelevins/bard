...
(define (bard:make-readtable)
  (let ((rt (##make-standard-readtable)))
    ...
    (macro-readtable-paren-keyword-set! rt 'bard:application)
    (macro-readtable-bracket-keyword-set! rt 'bard:sequence)
    ...
    (if colon-pos
        (let* ((mname (substring namestr 0 colon-pos))
          ...
          (cond 
           ...
           ((< (string-length mname) 1)(frame:put (bard:syntax 'name (string->symbol vname))
                                                  module-name: 'bard.keyword))
           ((not (%valid-module-name? mname))(error "Invalid module name" mname))
           (else (frame:put (bard:syntax 'name (string->symbol vname))
                            module-name: (string->symbol mname)))))
        (frame:put (bard:syntax 'name (string->symbol namestr))
                   module-name: #f))))
                    ...
                    (if (pair? read-output)
                        (let ((items (cdr read-output)))
                          (case (car read-output)
                            ((bard:sequence) (bard:syntax 'sequence items))
                            ((bard:application) (bard:syntax 'application items))
                            ((bard:frame) (bard:syntax 'frame items))
