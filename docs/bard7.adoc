...
Bard infers the function from the method definition. It finds the most specific existing class that already has `<small-integer>` as a member and uses that class for the definition:

----
bard.user> (function add [Integer Integer])
----

If it can't identify a most-specific class, then it declares that the function accepts only the structures on thiwhc it's specialized, like this:
...
Bard must now account for the fact that `x` and `y` might be either `<small-integer>` or `<string>`. Because the new structures are not members of `Integer` it adds another function definition:
