...
`<vector>` is a *member* of `List`. That means that there are some functions in the `bard.lists` protocol that accept arguments of type `List`, and at least one of those functions defines a method for a `List` argument of type `<vector>`.

Here's an example; the `<vector>` method for `any` could be defined like this:
