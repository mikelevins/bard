...
This entry says that `any` is a *function* that accepts a single argument of *class* `List`. So what's a *function* and what's a *class*?

A *function* is a procedure that inspects its arguments and, based on what it finds, decides what code to run. This decision-making process is called *dispatching*. Bard functions dispatch on all of their *required arguments*. When dispatching, a function can consider the types of its arguments, or their exact values, or the results of certain other tests that you can provide when defining *methods* on the function.
