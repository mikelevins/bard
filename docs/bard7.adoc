...
Bard's lexical syntax provides ways to write literal values of most of its built-in datatypes. In most cases, the simplext lexical syntax for a datatype is *nonspecific*; that is, it's defined to return an instance of a particular *class*, but not of any specific *structure*.

Writing the lexical syntax for a given datatype yields an instance of the *default structure* for that type. For example, the default structure for the `List` type is `<cons>`, so writing `(1 2 3)` returns a `<cons>`. You can specify that you want some particular structure using the function `as`:

....
(as <vector> '(1 2 3))
....

Bard provides a shorthand for `as`:

....
#<vector> '(1 2 3)
....

Alternatively, you could simply call the `vector` constructor:

....
(vector 1 2 3)
....

The shorthand syntax comes in handy when you want to contruct a value with a more complicate expression, and you want to specify the structure of the final result. 
...
Lists are sequences of values that are addressable by index. `List` is a class, not a structure, and its members include several different structures that represent sequences of values. Instances of `List` participate in the `Listing` protocol.
...
|===
|Examples |Structure |Notes
|{}, {:name "Fred" :age 35} |<dict> |Bard supports several
