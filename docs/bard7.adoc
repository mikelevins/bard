...
Still, Bard provides mutable structures for those cases where mutability is really needed. Nevertheless, it's an *immutability first* language: it encourages the use of immutable structures whenever they are practical.
...
Bard provides literal syntax for its most commonly-used datatypes. Most of the examples in the "Data" section are literal expressions for the listed types.

The table gives the *class* of each value, but not the *structure*. That's because Bard doesn't specify a required structure for each literal syntax; only a class. When you write a data literal, Bard constructs an instance of the *default structure* for the syntax. The next sections explains default structures in greater detail.

The default structure for the `List` type is `<cons>`, so writing `[1 2 3]` returns a `<cons>`.
...
That expression says to construct a `List` from the default structure, then convert it to the desired output, but Bard may rewrite the expression to avoid constructing and then converting the value. For example, Bard may rewrite this example as a direct call to the `vector` constructor:
...
For each literal data syntax there is a *default structure* defined by the implementation. The *default structure* is the structure that Bard uses to construct instances from literal expressions if you don't specify a different one.
...
In some of these cases more than one default structure is listed. That's because in some cases the default structure depends on the value represented by the syntax.
