...
`<vector>` is a *member* of `List`. That means that there's at least one function that accepts an argument of type `List`, and it defines a method the *specializes* that `List` argument on `<vector>`.
