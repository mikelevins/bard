...
(define (%makevm)
  (let ((fn #f)
    ...
    (%private-make-vm fn code pc instr stack nargs env globals #f)))
...
(define (%showvm vm)
  (let* ((fn (%fn vm))
         ...
         (pc (%pc vm))
         (instr (%instr vm))
         (instr-display (%instr->string instr))
         (stack (%stack vm))
         (nargs (%nargs vm))
         (env (%env vm))
         (globals (%globals vm)))
    (newline)
    (display "Bard VM 0.3.0")(newline)
    (display (str "pc: " pc " instr: " instr-display))(newline)
    (display (str "top of stack: " (if (null? stack) "-" (car stack))))
    (newline)))
...
(define (%runvm vm #!key (log #f))
  (if (%vm-code vm)
      (call/cc
       (lambda (exit)
         (%sethalt! vm (lambda (vm) (exit (%top vm))))
         (let loop ()
           (%stepvm vm)
           (loop))))
