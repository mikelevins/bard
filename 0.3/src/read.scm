...
(define (bard:make-readtable)
  (let ((rt (##make-standard-readtable)))
    ...
    (macro-readtable-paren-keyword-set! rt 'syntax:app)
    (macro-readtable-bracket-keyword-set! rt 'syntax:list)
    (macro-readtable-brace-keyword-set! rt 'syntax:table)
    rt))
...
(define (%read-cons val)
  (cond
   ...
   ((eq? 'syntax:app (car val)) (cond 
                                 ...
                                 (else (cons 'syntax:app (map %read-value->bard-syntax (cdr val))))))
   ((eq? 'syntax:list (car val)) (if (null? (cdr val)) 
                                     '(syntax:empty-list)
                                     (cons 'syntax:list (map %read-value->bard-syntax (cdr val)))))
   ((eq? 'syntax:table (car val)) (if (null? (cdr val)) 
                                      '(syntax:empty-table)
                                      (cons 'syntax:table (map %read-value->bard-syntax (cdr val)))))
   (else (map %read-value->bard-syntax val))))

(define (%read-value->bard-syntax val)
  (cond
   ((null? val) '(syntax:nothing))
   ((eq? 'undefined val) '(syntax:undefined))
   ((eq? 'nothing val) '(syntax:nothing))
   ((eq? 'true val) '(syntax:true))
   ((eq? 'false val) '(syntax:false))
   ((pair? val)(%read-cons val))
   ((char? val) `(syntax:character ,val))
   ((keyword? val) `(syntax:keyword ,val))
   ((number? val) `(syntax:number ,val))
